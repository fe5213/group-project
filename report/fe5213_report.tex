...
\paragraph{Breakdown.}
Distortionary labor-income tax financing breaks neutrality; Figure~\ref{fig:breakdown} compares steady states. Dynamic responses in Figures~\ref{fig:fin_c}--\ref{fig:fin_n} use the finite-horizon perfect-foresight procedure described in Section~\ref{sec:extension}.
...
\section{Extension: distortionary labor-income taxation}
\label{sec:extension}

This extension is kept separate from the Markov benchmark in Section~\ref{sec:ricardian}. When spending is financed by a proportional tax on labor income rather than a lump sum, the wedge $(1-\tau_t)$ enters the intratemporal first-order condition. The timing and level of taxes then affect marginal rates of substitution between consumption and leisure, so real allocations need not coincide across financing schemes that deliver the same $\{g_t\}$: lump-sum Ricardian neutrality generally fails once marginal tax rates distort labor supply and saving margins. Figure~\ref{fig:breakdown} contrasts steady states at common $g/Y$; Figures~\ref{fig:fin_c}--\ref{fig:fin_n} report dynamic responses.

\paragraph{Perfect-foresight transition solver.}
The panels in Figures~\ref{fig:fin_c}--\ref{fig:fin_n} are generated by a finite-horizon, perfect-foresight algorithm distinct from the Markov value-function iteration. For each financing regime, the unknowns are sequences $\{c_t,n_t,k_{t+1}\}_{t=0}^{T-1}$ satisfying (i) the goods-market/resource constraint, (ii) the intratemporal labor--consumption condition (with $\tau_t$ either lump-sum or tied to labor income to balance the budget each period), and (iii) the intertemporal Euler equation for capital, with terminal values anchored at the deterministic steady state appropriate to that regime. The two regimes are fed identical $\{g_t\}$ paths so that differences isolate the financing distortion rather than the spending impulse.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{irf_consumption_financing_regimes.png}
  \caption{Consumption: lump-sum versus labor-income tax financing (finite-horizon perfect foresight).}
  \label{fig:fin_c}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{irf_labor_financing_regimes.png}
  \caption{Labor: same financing comparison as Figure~\ref{fig:fin_c}.}
  \label{fig:fin_n}
\end{figure}
